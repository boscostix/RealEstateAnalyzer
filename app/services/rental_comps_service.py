"""Deterministic rental comparable service with cache and provider fallback."""
...
from app.research.comparables_base import RentalCompsProvider
from app.research.config import ResearchConfig
from app.services.research_provider_registry import ResearchProviderRegistry
from app.utils.comparables import filter_and_rank_rental_comps
from app.utils.research_validation import validate_provider_latency, validate_source_url


class RentalCompsService:
    """Coordinates rental comparable providers, ranking, fallback, and cache usage."""
    ...
    async def research(
        ...
        providers = self._registry.supported_providers(
            ResearchDomain.RENTAL_COMPS,
            request.property,
        )
        warnings: list[str] = []
        for provider in providers:
            if not isinstance(provider, RentalCompsProvider):
                continue
            ...
            ranked, summary, confidence = filter_and_rank_rental_comps(
                request.property,
                validated.data.comparables,
                request.filters,
            )
            latency_ms = int((time.perf_counter() - started_at) * 1000)
            response_result = ResearchResult[RentalCompsData](
            ...
            return RentalCompsResearchResponse(success=True, result=response_result)
        raise RentalCompsUnavailableError(
            message="No rental comparable providers could complete the request."
        )

    def _cache_key(self, request: RentalCompsResearchRequest) -> str:
        address = request.property.full_address.final_value or "unknown-address"
        return (
            f"rental_comps:{request.property.provider}:{address.lower()}:"
            f"{request.filters.model_dump_json()}"
        )

    def _validate_provider_result(
        self,
        result: ResearchResult[RentalCompsProviderData],
    ) -> ResearchResult[RentalCompsProviderData]:
        if result.metadata.domain != ResearchDomain.RENTAL_COMPS:
            raise ResearchValidationError(
                message="Research result domain must be rental_comps.",
                field="domain",
            )
    ...
    ) -> ResearchResult[RentalCompsData]:
